...
@section('content-header')
    <section class="content-header">
        <div class="pull-left">
            ...
            <a href="{{route('admin.comparing.index')}}" data-toggle="tooltip" title="" class="btn btn-default" data-original-title="لغو"><i class="fa fa-reply"></i></a></div>
        <h1>
            خصوصیت
    ...
    <section class="content">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form action="{{route('admin.comparing.update',$comparing)}}" class="form-horizontal" method="post" enctype="multipart/form-data" id="form-category">
                    {{csrf_field()}}
                    {{method_field('PATCH')}}
                    <div class="form-group required">
                        <label class="col-sm-2 control-label" for="name">نام خصوصیت</label>
                        <div class="col-sm-6">
                            <input id="name" name="name" value="{{$comparing->name}}" placeholder="نام خصوصیت"  class="form-control" type="text">
                    ...
                    <div class="form-group required">
                        ...
                        <div class="col-sm-6">
                            <textarea id="desc" name="desc"  placeholder="توضیحات"  class="form-control">{{$comparing->desc}}</textarea>
                    ...
                    <div class="form-group required">
                        ...
                        <div class="col-sm-6">
                            <select name="comparing_group_id[]" id="attribute_group_id" style="width: 100%" class="form-control typeselm" multiple>
                                @foreach($comparing_groups as $comparing_group)
                                    <option value="{{$comparing_group->id}}" @foreach($comparing->comparing_groups as  $att_g) @if($att_g->id==$comparing_group->id) selected @endif @endforeach  >{{$comparing_group->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-sm-6">
                            <select name="type" id="type" class="form-control typesel"  >
                                @foreach($options->all() as $option)
                                    <option value="{{$option['id']}}" @if($comparing->type==$option['id']) selected @endif>{{$option['name']}}</option>
                                @endforeach
                            </select>
...
@section('admin-footer')
    <script type="application/javascript">
        ...
        @if($comparing->comparing_options && $comparing->type !='1')
        $('#opt_place').empty();
        $('#opt_place').append(htmlopt_place);
        @forEach($comparing->comparing_options  as $option)
